...
class DatabasePORecord:
    ...
    def is_my_parent(self, other: POEntry) -> bool:
        """
        Return True if `other` (a polib.POEntry) has the same msgid and msgctxt
        as this record, by comparing SHA-256(msgid+msgctxt).
        """
        if not isinstance(other, POEntry):
            return False

        # combine and hash both sides
        mine = f"{self.msgid}{self.msgctxt}"
        theirs = f"{other.msgid}{other.msgctxt}"

        h1 = hashlib.sha256(mine.encode("utf-8")).hexdigest()
        h2 = hashlib.sha256(theirs.encode("utf-8")).hexdigest()
        return h1 == h2
    ...
    def retrieve_from_db(self) -> None:
        ...
        """
        from .translation_db import db
        rec = db.get_entry(self.msgid, self.msgctxt)
    ...
    def insert_to_db(self) -> None:
        ...
        rec = db.add_entry(self.msgid, self.msgctxt, initial=initial)
        self.unique_id = rec.unique_id
        self.msgstr_versions = rec.msgstr_versions
    ...
    def update_translation_version(self, translation: str) -> None:
        ...
        """
        from .translation_db import db
        rec = db.add_version(self.unique_id, translation)
        self.msgstr_versions = rec.msgstr_versions
    ...
    def delete_record(self) -> None:
        ...
        db.delete_entry(self.unique_id)
        self.unique_id = None
        self.msgid = None
        self.msgctxt = None
